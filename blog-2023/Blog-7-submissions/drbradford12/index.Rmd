...
R operates under the following:
...
ggpolt2 operates under the following:
...
The package dependencies are `pandas`, `requests`, `pdfplumber`, `datetime` and `os`  
...
Based on the fact that ggplot2 has a MIT license and can be used on the original GPL version. I believe that this can be done.
...
Based on the fact that we need to list the packages that our package is dependent on, I believe that this isn't something that can be done.
...
I'm not sure but I *believe* that most of the packages in the `tidyverse` package are MIT license versions, again I believe that as long as the package is being referenced in the dependencies we will avoid any infringement issues.
