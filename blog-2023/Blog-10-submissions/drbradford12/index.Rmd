...
- Respect website terms of service: use the `ROBOTS.TXT` file to ensure that we know, hat is allowed and what is disallowed.

- Obtain consent: Using the polite package, we can see if there are any reasons not to scrape a particular website.

- Minimize impact on the website: We don't want to upset the flow if traffic when we could be using a publicly available API.
...
The `ROBOTS.TXT` file, a.k.a. Robots Exclusion Standard, indicates the web-crawling software where it is allowed (or not allowed) within the website. 

An instance would be "Blocking a specific web crawler from a specific web page": 
